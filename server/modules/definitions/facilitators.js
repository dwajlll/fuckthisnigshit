...
exports.makeTurret = (type, options = {}) => {
    ...
    */
    ...
    for (let gun of GUNS) {
        ...
        gun.PROPERTIES.SHOOT_SETTINGS = exports.combineStats([gun.PROPERTIES.SHOOT_SETTINGS, g.half_recoil, ...extraStats])
    }
...
class LayeredBoss {
    ...
    addLayer({gun, turret}, decreaseSides = true, layerScale, MAX_CHILDREN) {
        ...
        let layer = {
            ...
            COLOR: -1,
            INDEPENDENT: true,
            FACING_TYPE: ["spin", { speed: 0.045 / Config.runSpeed * (this.layerID % 2 ? -1 : 1) }],
            MAX_CHILDREN, 
            GUNS: [],
            TURRETS: [],
        };
...
exports.makeRare = (type, level) => {
    ...
    return {
        PARENT: "food",
        LABEL: ["Shiny", "Legendary", "Shadow", "Rainbow", "Trans", "Unknown0"][level] + " " + type.LABEL,
        VALUE: [100, 500, 2000, 4000, 5000, 10000][level] * type.VALUE,
        SHAPE: type.SHAPE,
        SIZE: type.SIZE,
        COLOR: ["lightGreen", "teal", "darkGrey", "rainbow", "trans", "evening"][level],
        ALPHA: level == 2 ? 0.25 : 1,
        BODY: {
            DAMAGE: [1, 1, 2, 2.5, 2.5, 4][level] * type.BODY.DAMAGE,
            DENSITY: [1, 1, 2, 2.5, 2.5, 4][level] * type.BODY.DENSITY,
            HEALTH: [2, 4, 4, 6, 8, 16][level] * type.BODY.HEALTH,
            PENETRATION: [1.5, 1.5, 2, 2.5, 2.5, 3.5][level] * type.BODY.PENETRATION,
