...
pumpkinNestTick = tile => {
    if (++tile.data.enemySpawnCooldown > Config.ENEMY_SPAWN_COOLDOWN_PUMPKIN_NEST) {
        tile.data.enemySpawnCooldown = 0;
        if (tile.data.enemyCount < Config.ENEMY_CAP_PUMPKIN_NEST && Math.random() < Config.ENEMY_SPAWN_CHANCE_PUMPKIN_NEST) {
            spawnNatural(tile, Config.ENEMY_TYPES_PUMPKIN_NEST, 'enemy');
        }
    }
},
...
module.exports = { normal, nest, pune, wall, nestNoBoss };